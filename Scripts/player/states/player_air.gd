...
# if we want more realism at jump, tweakable. 0 = no movement in-air, player.SPEED = full movement in-air
@onready var air_control: float = player.SPEED
...
func physics_process(delta: float):
	...
	if not player.is_on_floor():
		...
		var target_vel_x = direction.x * player.SPEED
		var target_vel_z = direction.z * player.SPEED
