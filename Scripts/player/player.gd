...
@onready var camera_3d: Camera3D = (
	$HPivot/VPivot/SpringArm3D/Camera3D
)

@onready var spring_arm: SpringArm3D = (
	$HPivot/VPivot/SpringArm3D
)
...
# ================================================================
# PLAYER MODEL / SKELETON
# ================================================================

@onready var player_: Node3D = $player

@onready var metarig: Node3D = (
	$player/metarig
)

@onready var skeleton_3d: Skeleton3D = (
	$player/metarig/Skeleton3D
)

@onready var hand_r_001: BoneAttachment3D = (
	$player/metarig/Skeleton3D/hand_R_001
)

@onready var roundcube: MeshInstance3D = (
	$player/metarig/Skeleton3D/Roundcube
)
		...
		if lock_target_range.has_overlapping_bodies():
			...
			for body in lock_target_range.get_overlapping_bodies():
				...
				if body.is_in_group("enemy"):
					...
					var enemy_distance_to_center = (
						camera_vector.dot(enemy_vector)
					)
...
func _process(delta: float) -> void:
	...
	if drunk:
		...
		var wobble_x = (
			sin(drunk_camera_time * 2.5) *
			0.04
		)

		var wobble_z = (
			cos(drunk_camera_time * 2.0) *
			0.04
		)
		...
		spring_arm.position.y = (
			sin(drunk_camera_time * 3.0) *
			0.05
		)


		var fov_wobble = (
			sin(drunk_camera_time * 1.8) *
			4.0
		)
...
func _on_lock_target_range_body_entered(
	_body: Node3D
) -> void:
...
func _on_lock_target_range_body_exited(
	body: Node3D
) -> void:
...
func check_enemy_is_visible(
	closest_enemy: CharacterBody3D
) -> bool:
	...
	return (
		true
		if ray_result and
		ray_result.collider == closest_enemy
		else false
	)
...
# ================================================================
# ROTATE THE ACTUAL PLAYER MODEL
# ================================================================

func update_visuals_rotation(
	direction: Vector3,
	delta: float
) -> void:
	...
	if enemy_target:
		...
		direction_to_enemy = (
			direction_to_enemy.normalized()
		)


		target_angle = atan2(
			-direction_to_enemy.x,
			-direction_to_enemy.z
		)
...
func activate_parry_stun(
	...
	var attacker_state_machine = (
		attacker.get_node_or_null(
			"StateMachine"
		)
	)
	...
	for state in (
		attacker_state_machine.get_children()
	):

		if state.name.to_lower().ends_with(
			"stun"
		):
